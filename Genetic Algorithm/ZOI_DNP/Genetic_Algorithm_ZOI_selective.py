...
def new_generations(Gen, population_size):
    ...
    new = [selected, Compound_generation_ZOI_synergy_one_bac_drug.predict_ZOI_of_drug_np(Compound_generation_ZOI_synergy_one_bac_drug.population(half))]
    new_generation_input = pd.concat(new)
    new_generation_input.reset_index(drop=True, inplace=True)

    new_gen = Crossing_ZOI_selective.perform_evolution(new_generation_input, cross_over_rate, mutation_rate)
...
def Genetic_Algorithm(generation_number, population_size):
    Generation1 = Compound_generation_ZOI_synergy_one_bac_drug.predict_ZOI_of_drug_np(Compound_generation_ZOI_synergy_one_bac_drug.population(population_size)).sort_values('Fitness', ascending=False)
    ...
    Generation2 = Crossing_ZOI_selective.perform_evolution(Generation1, cross_over_rate, mutation_rate).sort_values('Fitness', ascending=False)
